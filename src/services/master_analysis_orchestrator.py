...
class MasterAnalysisOrchestrator:
    ...
    def _execute_phase_1_massive_collection(
        ...
        try:
            ...
            # INTEGRAÇÃO PREDITIVA: Análise inicial dos dados coletados
            if HAS_PREDICTIVE_SERVICE:
                try:
                    logger.info("🔮 Executando análise preditiva inicial dos dados coletados...")
                    
                    # Prepara dados coletados para análise
                    dados_para_analise = []
                    if "web_results" in massive_data:
                        dados_para_analise.extend(massive_data["web_results"])
                    if "social_results" in massive_data:
                        dados_para_analise.extend(massive_data["social_results"])
                    if "youtube_results" in massive_data:
                        dados_para_analise.extend(massive_data["youtube_results"])
                    ...
                    asyncio.set_event_loop(loop)
                    try:
                        insights_iniciais = loop.run_until_complete(
                            predictive_analytics_service.analyze_initial_data(dados_para_analise)
                        )
                        
                        # Salva insights iniciais
                        salvar_etapa("insights_iniciais_pos_coleta", insights_iniciais, "insights_parciais", session_id)
                        
                        # Adiciona insights ao contexto para próximas fases
                        massive_data["predictive_insights_initial"] = insights_iniciais
                        
                        logger.info("✅ Análise preditiva inicial concluída")
                        
                    finally:
                        loop.close()
                        
                except Exception as e:
                    logger.warning(f"⚠️ Erro na análise preditiva inicial: {e}")
    ...
    def _execute_phase_4_report_generation(
        ...
        try:
            ...
            # INTEGRAÇÃO PREDITIVA: Análise final da sessão
            insights_finais = None
            if HAS_PREDICTIVE_SERVICE:
                try:
                    logger.info("🔮 Executando análise preditiva final da sessão...")
                    
                    import asyncio
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    try:
                        insights_finais = loop.run_until_complete(
                            predictive_analytics_service.analyze_session(session_id)
                        )
                        
                        # Salva insights finais
                        salvar_etapa("insights_finais_pre_relatorio", insights_finais, "insights_finais", session_id)
                        
                        logger.info("✅ Análise preditiva final concluída")
